'use client';
import { validateEmail, validatePass } from '@/lib/validateInput';
    ...
    return (
        <div className='flex flex-col justify-center items-center w-[50%] mt-20 px-10 py-8 border-1 rounded-xl shadow-md shadow-black/10 max-lg:w-full '>
            <form onSubmit={submitHandler} className='flex flex-col justify-center items-start gap-3 w-full mt-4' >
                ...
                <input ref={inputRef} onChange={onChangeHandler} value={email.value} type='email' name='email' placeholder='Email*' className={`w-full text-sm font-sans font-normal leading-5 px-3 py-3 rounded-md shadow-md shadow-slate-500 focus:shadow-outline-purple focus:shadow-lg hover:border-blue-500  bg-white text-slate-900 outline-none border border-solid ${ email.isValid ? 'focus:border-blue-500' : 'focus:border-red-500' } ${ email.isValid ? 'border-slate-300' : 'border-red-300 placeholder:text-red-500' }`} />
                ...
                <div className='w-full relative'>
                    <div onClick={() => setShowPass(!showPass)} className='hover:cursor-pointer' >
                        {showPass ? ( <IoMdEye className='absolute right-4 translate-y-[100%] text-slate-600' /> ) : ( <IoMdEyeOff className='absolute right-4 translate-y-[100%] text-slate-600' /> )}
                    </div>
                ...
                <button type='submit' className='mt-6 w-full px-4 py-3 text-white text-lg font-semibold bg-black rounded-lg ' > {formState.loading ? ( <PropagateLoader color='white' className='mb-[14px] scale-75' /> ) : ( 'Log In' )} </button>
            </form>
        </div>
    );
};
