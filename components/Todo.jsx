...
const Todo = ({  todo, deleteTodo, editHandler, toggleComplete, onChangeHandler, editingId, editingTodo, submitUpdateHandler, isLoading}) => {
    ...
    return (
      <div className={`text-white flex justify-center items-center mb-4 p-4  rounded-xl hover:scale-[1.02] transition-all  ${todo.isComplete ? 'bg-emerald-600' : 'bg-slate-800'}`}>
              ...
              {
                  isLoading.loading && todo.id === isLoading.id ? <ClipLoader className="p-6 " color="white"/> :
                  <>
